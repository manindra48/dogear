"""Command-line interface for Dogear."""
...
def _cmd_find(args):
    ...
    if not results:
        print("no results (is the index empty? run: dogear index <path-to-bookmarks.html>)")
        return 1
...
def build_parser():
    p = argparse.ArgumentParser(
        prog="dogear",
        description="Dogear — local semantic search over your browser bookmarks.",
    )
    p.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    p.add_argument(
        "--db", default=os.environ.get("DOGEAR_DB"),
