...
def default_db_path() -> Path:
    env = os.environ.get("DOGEAR_HOME")
    if env:
        base = Path(env)
    elif os.name == "nt":
        # On Windows, use %LOCALAPPDATA%\dogear (dotfolders are unusual)
        local = os.environ.get("LOCALAPPDATA")
        base = Path(local) / "dogear" if local else Path.home() / ".dogear"
    else:
        base = Path.home() / ".dogear"
